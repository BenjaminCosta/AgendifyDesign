...
function extendMaterial(cfg: ExtendedMaterialConfig) {
...
const MergedPlanes = forwardRef<THREE.Mesh, PlaneProps>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    const shaderMaterial = mesh.current?.material as THREE.ShaderMaterial | undefined;
...
const DirLight = ({ position, color }: DirLightProps) => {
  ...
  useEffect(() => {
    const directionalLight = dir.current;
